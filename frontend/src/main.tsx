...
function PublicSite({ language, setLanguage }: { language: Language; setLanguage: (language: Language) => void }) {
  ...
  return <main className="public-site"><nav className="public-nav"><strong>MARICS<span>/ SECURITY</span></strong><div className="public-links"><a href="#how-it-works">{t.nav.howItWorks}</a><a href="#features">{t.nav.features}</a><a href="#organizations">{t.nav.organizations}</a><a href="#pricing">{t.nav.pricing}</a></div><select aria-label="Language" value={language} onChange={(event) => setLanguage(event.target.value as Language)}><option value="en">EN</option><option value="af">AF</option><option value="pt">PT</option></select><button onClick={() => setShowAuth(true)}>{t.nav.login}</button></nav><section className="public-hero"><p className="eyebrow">{t.landing.eyebrow}</p><h1>{t.landing.title}</h1><p className="public-lede">{t.landing.body}</p><div className="public-actions"><button className="primary" onClick={() => setShowAuth(true)}>{t.landing.primary} <b>→</b></button><a href="#how-it-works">{t.landing.secondary} <span>↓</span></a></div></section><section className="public-intro"><div><p className="eyebrow">01 / MARICS METHOD</p><h2>{t.landing.intro}</h2></div><p>{t.landing.introBody}</p></section><section id="how-it-works" className="public-steps">{t.landing.features.map((feature, index) => <article key={feature}><span>0{index + 1}</span><h3>{feature}</h3><p>{t.landing.introBody}</p></article>)}</section><section id="features" className="public-cta"><p className="eyebrow">MARICS / SECURITY</p><h2>{t.landing.cta}</h2><button className="primary" onClick={() => setShowAuth(true)}>{t.landing.primary} <b>→</b></button></section><footer>{t.landing.footer}<span>{t.nav.about} · {t.nav.demo} · {t.nav.signup}</span></footer></main>;
}
